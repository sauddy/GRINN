...
def plot_function(net,time_array,initial_params,velocity = False,isplot =False ,animation=False):
    ...
    for t in time_array:
        ...
        X = np.linspace(xmin,xmax,200).reshape(200, 1)
        t_ = t*np.ones(200).reshape(200, 1)
        ...
        output_0 = net(pt_x_collocation,pt_t_collocation)
        
        rho_pred0 = output_0[:,0:1].data.cpu().numpy()
        v_pred0 = output_0[:, 1:2].data.cpu().numpy()
