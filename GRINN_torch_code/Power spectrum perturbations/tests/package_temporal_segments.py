"""
Bundle the segmented PINN test harness into a single Kaggle-friendly script.

Running this script produces ``kaggle_temporal_segments.py`` which embeds the
core modules plus ``tests/test_temporal_segments.py``. Upload the generated
file together with ``config.py`` on Kaggle to run the overlapping-window test
without recreating the repo directory structure.
"""
...
OUTPUT_FILE = ROOT / "kaggle_temporal_segments.py"
...
MODULE_SPECS: List[Tuple[str, str]] = [
    ...
    ("tests/test_temporal_segments.py", "tests.test_temporal_segments"),
]
...
def _build_combined_script() -> str:
    header_lines = [
        "# Auto-generated by package_temporal_segments.py",
    ...
    for rel_path, module_name in MODULE_SPECS:
        ...
        body_lines.append(f"_register_module({module_name!r}, [{attr_list}])")
        body_lines.append("")
...
def main() -> None:
    script_text = _build_combined_script()
