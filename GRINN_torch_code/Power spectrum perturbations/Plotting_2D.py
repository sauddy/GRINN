...
def Two_D_surface_plots(net, time, initial_params, ax=None):
    """
...
def create_2d_surface_plots(net, initial_params, time_points=None):
    """
    ...
    Args:
        net: Trained neural network
        initial_params: Tuple containing (xmin, xmax, ymin, ymax, rho_1, alpha, lam, output_folder, tmax)
        time_points: List of time points to plot (default: [0.0, 0.3, 0.6, 1.0, 1.5])
    """
    if time_points is None:
        time_points = [0.0, 0.3, 0.6, 1.0, 1.5]
    ...
    for i, t in enumerate(time_points):
        if i < len(axes):
            ...
            Two_D_surface_plots(net, t, initial_params, ax=axes[i])
    ...
    for t in time_points:
        print(f"Creating individual plot at t = {t}")
        Two_D_surface_plots(net, t, initial_params, ax=None)
        plt.show()
        plt.close()

    print("All 2D surface plots displayed successfully!")
...
def create_all_plots(net, initial_params, include_growth=True):
    """
    Create all plots: 2D surface plots, 1D comparison plots, and optionally growth comparison
    
    Args:
        net: Trained neural network
        initial_params: Tuple containing (xmin, xmax, ymin, ymax, rho_1, alpha, lam, output_folder, tmax)
        include_growth: Whether to include the slow growth comparison plot
    """
    print("="*60)
    print("CREATING ALL VISUALIZATION PLOTS")
    print("="*60)
    
    # 1. Create 2D surface plots
    create_2d_surface_plots(net, initial_params)
    
    # 2. Create 1D comparison plots
    create_1d_comparison_plots(net, initial_params)
    
    # 3. Create growth comparison plot (optional, can be slow)
    if include_growth:
        print("Creating growth comparison plot (this may take a while)...")
        create_growth_comparison_plot(net, initial_params)
    else:
        print("Skipping growth comparison plot for faster execution")
